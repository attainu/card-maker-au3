...
import { Collapse, Button, CardBody, Card } from "mdbreact";
import {
  MDBContainer,
  MDBRow,
  MDBCol,
  ...
  const handleSubmit = async event => {
    ...
    try {
      const result = await axios.post(
        "http://localhost:5000/api/auth/emailSender",
        {
          sender: cardData.email,
  ...
  const fetch = async () => {
    try {
      const CardDetails = await axios.get(
        `http://localhost:5000/api/jade/${props.match.params.id}/show`
      );
  ...
  return (
    <div>
      ...
      <MDBContainer>
        <MDBRow>
          <MDBCol size="4">
            <JadeCard val={cardData} />
          </MDBCol>
          <MDBCol size="8">
            <div className="manualtoggleform offset-md-3">
              <Button
                color="primary"
                onClick={toggle}
                style={{ marginBottom: "1rem", width: "550px", height: "35px" }}
                className="toggleform"
              >
                <MDBIcon className="downicon" icon="angle-down">
                  &nbsp;Leave a message
                </MDBIcon>
              </Button>
